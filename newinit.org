...
Emacs' default keybindings are linked with repetitive strain injury. Boon emphasizes modal editing (similar to Vim), key sequences, and composability to avoid the strain associated with pressing key chords.
#+begin_src emacs-lisp  
(use-package evil
  :ensure t
  :pin melpa
